...
class MixUp(object):
    ...
    def __call__(self, batch):
        images_q, targets_q, records_q, images_k, targets_k, records_k, indicators, labeltypes, indices = zip(*batch)
        images_q = torch.stack(images_q)
        images_k = torch.stack(images_k)

        mixed_images_q, randind, lam = mixup(images_q, self.alpha)
        mixed_images_k, _, _ = mixup(images_k, self.alpha)
        mixed_targets_q = (torch.stack(targets_q), torch.stack(targets_q)[randind], lam)
        mixed_targets_k = (torch.stack(targets_k), torch.stack(targets_k)[randind], lam)
        mixed_indicators = (torch.tensor(indicators), torch.tensor(indicators)[randind])
        mixed_labeltypes = (torch.tensor(labeltypes), torch.tensor(labeltypes)[randind])

        return mixed_images_q, mixed_targets_q, mixed_images_k, mixed_targets_k, mixed_indicators, mixed_labeltypes, indices