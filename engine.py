...
def train_one_epoch_semi(model_student: torch.nn.Module, model_teacher: torch.nn.Module, criterion: torch.nn.Module,
    ...
    for _ in metric_logger.log_every(range(len(data_loader_label)), print_freq, header):
        ...
        air_targets = [d for d in targets if d.get("perspective") == "aerial"]
        ground_targets = [d for d in targets if d.get("perspective") == "ground"]

        loss_dict_ground = criterion(outputs, ground_targets, indicators_ground)
        loss_dict_ground['labels'] = loss_dict_ground['labels'] *(1 - lam)
        loss_dict_aerial = criterion(outputs, air_targets, indicators_aerial)
        loss_dict_aerial['labels'] = loss_dict_aerial['labels'] *(lam)
