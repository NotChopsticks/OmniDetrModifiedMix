...
def collate_fn(batch):
    ...
    if len(batch) > 4:
        ...
        batch[6] = nested_tensor_from_tensor_list(batch[6])
        batch[9] = nested_tensor_from_tensor_list(batch[9])
    return tuple(batch)
...
def interpolate(input, size=None, scale_factor=None, mode="nearest", align_corners=None):
    ...
    if float(torchvision.__version__.split('.')[1]) < 7:
        ...
        # if float(torchvision.__version__[:3]) < 0.5:s
        if float(torchvision.__version__.split('.')[1]) < 5:
